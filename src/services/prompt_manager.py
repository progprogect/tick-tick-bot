...
- complete_task: Отметить задачу как выполненную (для команд типа "отметь задачу X выполненной", "заверши задачу Y")
...
Для простых команд (одно действие) - используй старый формат:
{
  ...
  "dueDate": "дата в ISO 8601 в часовом поясе МСК (UTC+3), например: 2025-11-28T12:00:00+03:00",
  "priority": 0-3,
  "tags": ["тег1", "тег2"],
  "notes": "текст заметки",
  "recurrence": {"type": "daily|weekly|monthly", "interval": 1},
  "reminder": "дата и время в ISO 8601 в часовом поясе МСК (UTC+3), например: 2025-11-28T12:00:00+03:00",
  "period": "week|month|year|today|tomorrow",
  "startDate": "начальная дата для фильтрации (ISO 8601 в МСК, UTC+3)",
  "endDate": "конечная дата для фильтрации (ISO 8601 в МСК, UTC+3)"
}
...
9. "Отметь задачу опубликовать проект выполненной" (простая команда):
{
  "action": "complete_task",
  "title": "опубликовать проект"
}

10. "Заверши задачи X и Y" (множественное завершение):
{
  "operations": [
    {
      "type": "complete_task",
...
КРИТИЧЕСКИ ВАЖНО ДЛЯ ДАТ И ВРЕМЕНИ:
- ВСЕ даты и время должны быть в часовом поясе МСК (UTC+3)
- Если пользователь указывает время (например, "в 12:00", "в 12 00", "28 ноября в 12 00"), это время в МСК
- Формат дат: "YYYY-MM-DDTHH:MM:SS+03:00" (например: "2025-11-28T12:00:00+03:00")
- НЕ используй UTC (UTC+0) или другие часовые пояса - только МСК (UTC+3)
...
6. "Отметь задачу опубликовать проект выполненной":
{
...
7. "Заверши задачу подготовить отчет":
{
  "action_type": "complete_task",
  "required_data": {
    "task_by_title": ["подготовить отчет"]
...
8. "Создай задачу тест":
{
  "action_type": "create_task",
  "required_data": {}
}

9. "Какие задачи у меня на сегодня?":
{
...
10. "Покажи мои задачи на завтра":
{
  "action_type": "list_tasks",
  "required_data": {
    "tasks_by_filters": {
...
11. "Какую задачу я создал последней?":
{
  "action_type": "list_tasks",
  "required_data": {
    "tasks_by_filters": {
...
КРИТИЧЕСКИ ВАЖНО для bulk_move с просроченными задачами:
- Если команда содержит слова "просроченные", "просрочено", "просроченные задачи" - это ОБЯЗАТЕЛЬНО означает status: -1
- НИКОГДА не используй end_date или start_date для просроченных задач
- НИКОГДА не используй status: 0 вместе с end_date для просроченных задач
- status: -1 - это специальный код для просроченных задач (задачи с dueDate < текущая дата)
- Формат ДОЛЖЕН быть ТОЛЬКО: {"status": -1} - БЕЗ других полей в tasks_by_filters
- Пример ПРАВИЛЬНОГО запроса для просроченных задач:
  {"tasks_by_filters": {"status": -1}}
- Пример НЕПРАВИЛЬНОГО запроса (НЕ ДЕЛАЙ ТАК):
  {"tasks_by_filters": {"status": 0, "end_date": "2024-11-04"}}  ← ЭТО НЕПРАВИЛЬНО!
...
- Если команда содержит "на сегодня", "сегодня" - ОБЯЗАТЕЛЬНО укажи startDate и endDate для текущей даты ({current_date})
  Формат: startDate = "{current_date}T00:00:00+00:00", endDate = "{current_date}T23:59:59+00:00"
...
{{
  ...
  "targetProjectId": "ID целевого списка (для переноса)",
  "projectName": "название проекта (для create_project/delete_project - извлекай из команды)",
  "projectColor": "цвет проекта в формате #RRGGBB (для create_project)",
  "projectViewMode": "режим отображения: list или kanban (для create_project)",
  "projectKind": "тип проекта: TASK или NOTE (для create_project)",
  "dueDate": "дата в ISO 8601 в часовом поясе МСК (UTC+3), например: 2025-11-28T12:00:00+03:00",
  "priority": 0-3,
  "tags": ["тег1", "тег2"],
  "notes": "текст заметки",
...
ВАЖНО ДЛЯ ДАТ:
- ВСЕГДА используй часовой пояс МСК (UTC+3) для всех дат и времени
- Формат: "YYYY-MM-DDTHH:MM:SS+03:00" (например: "2025-11-28T12:00:00+03:00")
- Если пользователь указывает время (например, "в 12:00", "в 12 00", "28 ноября в 12 00"), это время в МСК
- НЕ используй UTC (UTC+0) или другие часовые пояса - только МСК (UTC+3)
- Примеры правильных форматов:
  * "2025-11-28T12:00:00+03:00" (28 ноября в 12:00 МСК)
  * "2025-11-28T00:00:00+03:00" (28 ноября в 00:00 МСК)
  * "2024-11-06T00:00:00+03:00" (6 ноября в 00:00 МСК)""",
...
- Примеры правильных форматов:
  * "2025-11-28T12:00:00+03:00" (28 ноября в 12:00 МСК)
  * "2025-11-28T15:30:00+03:00" (28 ноября в 15:30 МСК)
...
Команда: "Отметь задачу опубликовать проект выполненной"
Данные: {"tasks": {"опубликовать проект": {"id": "task_123", "projectId": "inbox123456"}}}
Ответ:
{
  "action": "complete_task",
  "taskId": "task_123",
...
ПРИМЕР для complete_task (только название):
Команда: "Заверши задачу подготовить отчет"
Данные: {"tasks": {"подготовить отчет": {"id": "task_456", "projectId": "inbox789012"}}}
Ответ:
{
  "action": "complete_task",
  "taskId": "task_456",
